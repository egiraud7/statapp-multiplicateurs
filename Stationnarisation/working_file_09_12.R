...
# Pour toutes les autres colonnes (plotter les différents pays en même temps)
...
for (i in 1:ncol(stationary_France)) {
  ts.plot(stationary_France[, i], gpars = list(main = paste("France", (colnames(stationary_France)[i])), ylab=""))
}

for (i in 1:ncol(stationary_Germany)) {
  ts.plot(stationary_Germany[, i], gpars = list(main = paste("Germany", (colnames(stationary_Germany)[i])), ylab=""))
}
